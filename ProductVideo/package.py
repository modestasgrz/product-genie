...
def run():
    ...
    zip_entries = gather_files('productvideo')

    for module in get_required_modules():
        module = importlib.import_module(module)
        zip_entries += gather_files(os.path.dirname(module.__file__), os.path.join('productvideo', 'modules'))

    filename = f'productvideo-{productvideo.__version__}.zip'
