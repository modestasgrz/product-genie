...
def shifBonestAnimationData(obj, selected_pose_bones, shift):
    ...
    for bone in selected_pose_bones:
        bone_name = 'pose.bones["{}"].'.format(bone.name)
...
import os
MAPS_JSON_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'properties', 'maps.json')
    ...
    if action:
        for fcurve in action.fcurves:
            for keyframe in fcurve.keyframe_points:
                keyframe.interpolation = interpolation  # Options: 'BEZIER', 'LINEAR', 'CONSTANT'
...
class ApplyMovementAnimationOperator(Operator):
    ...
    def execute(self, context):
        ...
        camera_action = MOVEMENT_ACTION_MAP[
            productvideo_addon_properties.MOVEMENT]['CAMERA']
        object_action = MOVEMENT_ACTION_MAP[
            productvideo_addon_properties.MOVEMENT]['OBJECT']
        ...
        if object_action:
            ...
            # Add a new NLA strip and assign the action to it
            nla_strip = nla_track.strips.new(object_action.name,
                                             start=1,
            ...
            # print(
            #     f"NLA Strip '{nla_strip.name}' created for action '{object_action.name}'"
            # )

        bpy.data.materials["GRADIENT"].node_tree.nodes["RGB"].outputs[
            0].default_value[
                0] = productvideo_addon_properties.ENVIRONMENT_COLOR[0]
        bpy.data.materials["GRADIENT"].node_tree.nodes["RGB"].outputs[
            0].default_value[
                1] = productvideo_addon_properties.ENVIRONMENT_COLOR[1]
        bpy.data.materials["GRADIENT"].node_tree.nodes["RGB"].outputs[
            0].default_value[
                2] = productvideo_addon_properties.ENVIRONMENT_COLOR[2]
    ...
    for obj in objects:
        ...
        if obj.type == 'MESH':
            ...
            for corner in bbox_world:
                min_coords = Vector(
                    (min(min_coords.x, corner.x), min(min_coords.y, corner.y),
                     min(min_coords.z, corner.z)))
                max_coords = Vector(
                    (max(max_coords.x, corner.x), max(max_coords.y, corner.y),
...
class ImportProductObject(Operator, ImportHelper):
    ...
    def set_origin_to_geometry(self, obj):
        ...
        bbox_center = sum(
            (mathutils.Vector(corner)
             for corner in obj.bound_box), mathutils.Vector()) / 8
    ...
    def execute(self, context):
        ...
        combined_dimensions, bbox_center = get_combined_bounding_box(
            imported_objs)

        max_dimension = max(combined_dimensions[0], combined_dimensions[1],
                            combined_dimensions[2])
        ...
        for obj in imported_objs:
            ...
            if obj.data:
                obj.data.transform(
                    obj.matrix_world.inverted() @ new_origin_matrix)
                obj.matrix_world = new_origin_matrix
